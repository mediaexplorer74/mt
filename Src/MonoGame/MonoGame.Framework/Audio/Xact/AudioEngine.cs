...
namespace Microsoft.Xna.Framework.Audio
{
	...
	/// 
	/// Applications use the methods of the audio engine to instantiate and manipulate core audio objects.
    ...
    public class AudioEngine : IDisposable
	{
		...
		///</summary>
		public const int ContentVersion = 39;//46;
        internal uint engineHandle = uint.MaxValue;
        internal int destChannelCount;
        internal static bool hasHookedDestroy;
        internal IntPtr pSettingsData;
        internal IntPtr p3dAudio;
        internal IntPtr pDsp;
        private bool _isDisposed;
        private static Dictionary<uint, WeakReference> audioInstances = new Dictionary<uint, WeakReference>();
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", Justification = "Parameter is here for compatibility with Windows platform.")]
        /// <param name="settingsFile">Path to a XACT settings file.</param>
		...
		public AudioEngine (string settingsFile, TimeSpan lookAheadTime, string rendererId)
		{
            ...
            string str = !string.IsNullOrEmpty(settingsFile) 
				? Path.GetFullPath(settingsFile)
				: throw new ArgumentNullException(nameof(settingsFile), "NullNotAllowed");

            //************************************************************************************
            //RnD
            /*
            using (BinaryReader binaryReader = new BinaryReader((Stream)File.OpenRead(str)))
            {
                bool flag = false;
                if (binaryReader.BaseStream.Length > 4L)
                {
                    byte[] numArray = binaryReader.ReadBytes(4);
                    flag = numArray[3] == (byte)70 && numArray[2] == (byte)83 && numArray[1] == (byte)71 && numArray[0] == (byte)88;
                }
                if (!flag)
                    throw new ArgumentException("InvalidContentVersion");
            }
            this.engineHandle = UnsafeNativeMethods.Engine.CreateHandle(str, str.Length, (uint)lookAheadTime.TotalMilliseconds, 
                rendererId, out this.destChannelCount, out this.pSettingsData, out this.p3dAudio);

            if (this.engineHandle == uint.MaxValue)
                throw new InvalidOperationException("CouldNotCreateResource");
            if (AudioEngine.hasHookedDestroy)
                return;
           
            
            //UnsafeNativeMethods.CueDestroyed += (CueDestroyedEventHandler)(pCue 
            //    => AudioEngine.NotifyDestroyedCue(pCue));
            */
            AudioEngine.hasHookedDestroy = true;
            //************************************************************************************
        ...
        internal static void AddInstance(uint waveBankHandle, object v)
        {
