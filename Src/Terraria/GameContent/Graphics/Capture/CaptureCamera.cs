...
namespace GameManager.Graphics.Capture
{
    internal class CaptureCamera
    {
        ...
        public CaptureCamera(GraphicsDevice graphics)
        {
            ...
            try
            {
                _frameBuffer = new RenderTarget2D(graphics, 2048, 2048, 
                    false, graphics.PresentationParameters.BackBufferFormat, DepthFormat.Depth24);
            }
            catch
            {
        ...
        public void Capture(CaptureSettings settings)
        {
            ...
            if (settings.UseScaling)
            {
                ...
                _scaledFrameBuffer = new RenderTarget2D(_graphics, num2, num2, false, 
                    _graphics.PresentationParameters.BackBufferFormat, DepthFormat.Depth24);
            }
            else
            ...
            while (x1 < rectangle.X + rectangle.Width)
            {
                ...
                while (y1 < rectangle.Y + rectangle.Height)
                {
                    ...
                    _renderQueue.Enqueue(new CaptureChunk(new 
                        Microsoft.Xna.Framework.Rectangle(x1, y1, width1, height1), 
